    ...
    <v-sheet class="px-8">
        ...
        <v-data-table
            ...
            <template v-slot:item.crypto="{ item }">
                <span class="text-caption">{{ item.value.crypto ? item.value.crypto.name : item.value.item }}</span>
            </template>
            ...
            <template v-slot:item.view="{ item }">
                <CryptoTradeDetail :order="item.value"></CryptoTradeDetail>
            </template>
...
<script>
  ...
  import CryptoTradeDetail from '@/components/CryptoTradeDetail.vue';
  ...
  export default {
    ...
    data: () => ({
        isLoading: false,
        processEvent: "process_crypto_trade_event",
        completeEvent: "complete_crypto_trade_event",
        rejectEvent: "reject_crypto_trade_event",
        updateEvent: "update_crypto_trade_event",
        utils: new Utils(),
        snackbar: false,
        text: ``,
        search: '',
    ...
    methods: {
        ...
        updateStatus(id,status,index){
            ...
            let http = new APIRequest();
            self.isLoading = true;
            let url = `${http.baseUrl}/orders/${id}`;
