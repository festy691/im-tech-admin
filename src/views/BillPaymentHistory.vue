<template>
    <v-sheet class="px-8">
        ...
        <v-data-table
            ...
            <template v-slot:item.balanceBeforeTransaction="{ item }">
                <span class="text-caption">₦{{ utils.formatMoney(item.value.balanceBeforeTransaction) }}</span>
            </template>
            ...
            <template v-slot:item.provider="{ item }">
                <span class="text-caption">{{ item.value.provider }}</span>
            </template>
            <template v-slot:item.beneficiary="{ item }">
                <span class="text-caption">{{ item.value.beneficiary }}</span>
            </template>
  ...
  export default {
    ...
    data: () => ({
        ...
        headers: [
          ...
          { title: 'Balance before', key: 'balanceBeforeTransaction' },
          { title: 'Balance after', key: 'balanceAfterTransaction' },
    ...
    methods: {
        getFundHistory(){
            if(sessionStorage.getItem('login')){
                ...
                let self = this;
                let url = `${http.baseUrl}/bills/transactions`;
                self.isLoading = true;
