  ...
  export default {
    ...
    methods: {
      ...
      async addProduct(){
        ...
        if(self.item.atmAddress)form.append('atmAddress',self.item.atmAddress);
        if(self.image) form.append('image', self.image.file);
  
        let http = new APIRequest();
        self.isLoading = true;
        let url = `${http.baseUrl}/cryptos/${self.item._id}`;
        http.putForm(url, form, (err,data)=> {
          ...
          this.emitter.emit("update-crypto", data);
          self.snackbar = true;
        });
      },
